...
- **FPGA_READY_PIN**(**Optional**, [Pin](https://esphome.io/guides/configuration-types.html#config-pin)): Input pin connected to the FPGA ready/reset indicator. When provided, the driver uses it to detect FPGA resets (falling edge or low-at-boot) and resync the display state. Defaults to `35`.
